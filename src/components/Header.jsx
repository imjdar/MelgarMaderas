...
export const Header = ({ whatsappNumber, onNavigate }) => {
  ...
  return (
    <header className={`glass-header ${isScrolled ? 'scrolled' : ''}`}>
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '80px' }}>
        ...
        <a href="#inicio" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', textDecoration: 'none' }}>
          <img 
            ...
            onError={(e) => {
              // Fallback to symbol logo if text logo unavailable
              e.target.src = '/assets/branding/logo-symbol.png';
            }}
          />
        </a>
        ...
        <nav className="desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          {navLinks.map((link) => (
            <a
              ...
              onMouseEnter={(e) => (e.target.style.color = 'var(--color-green-primary)')}
              onMouseLeave={(e) => (e.target.style.color = 'var(--color-wood-dark)')}
            >
              {link.label}
            </a>
          ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <a
            href={getWhatsAppQuoteUrl(whatsappNumber, 'Consulta General')}
            target="_blank"
      ...
      {/* Responsive Inline CSS helper for Mobile toggle */}
      <style>{`
