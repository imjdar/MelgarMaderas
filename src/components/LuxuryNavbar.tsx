...
export function LuxuryNavbar({ whatsappNumber, cartCount = 0, onOpenCart }: LuxuryNavbarProps) {
  ...
  return (
    <>
      <nav
        className="fixed top-0 left-0 w-full z-50 transition-all duration-500 bg-[#120C08] shadow-md py-0"
      ...
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 flex justify-between items-center min-h-[40px]">
        ...
        <div className="hidden lg:flex items-center justify-center space-x-8 w-2/4">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-xs tracking-wide text-white hover:text-[#C59B27] transition-colors uppercase"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="hidden lg:flex items-center justify-end space-x-6 w-1/4">
          ...
          <button
            onClick={onOpenCart}
            className="relative flex items-center justify-center p-1 text-white hover:text-[#C59B27] transition-colors"
      ...
      {/* Mobile Menu */}
      <div 
        className={`lg:hidden fixed inset-0 bg-[#120C08] z-40 flex flex-col items-center justify-center space-y-8 transition-transform duration-500 ${
      ...
      >
        ...
        {navLinks.map((link) => (
          <a
            ...
            href={link.href}
            onClick={() => setMobileMenuOpen(false)}
            className="text-2xl tracking-widest text-white hover:text-[#C59B27]"
        ...
        <div className="flex space-x-6 mt-12 text-white">
          <a href="#" aria-label="Instagram"><Instagram size={24} /></a>
          <a href="#" aria-label="Facebook"><Facebook size={24} /></a>
        </div>
      </div>
      </nav>
      <div className="h-[80px] md:h-[96px] w-full bg-[#120C08]" aria-hidden="true" />
    </>
  );
}
