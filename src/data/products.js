// Dataset Oficial de Productos de Maderas Melgar (melgarmaderas.com.ec)
...
// Helper to generate dynamic WhatsApp quotation URLs
export const getWhatsAppQuoteUrl = (whatsappNumber, productName, productId) => {
  // Clean phone number or default placeholder
  const phone = whatsappNumber ? whatsappNumber.replace(/[^0-9]/g, '') : '';
  const domain = 'melgarmaderas.com.ec';
  const text = encodeURIComponent(
    `Hola Maderas Melgar 👋, vi en su sitio web (${domain}) el producto "*${productName}*" y me gustaría solicitar una cotización personalizada y detalles de fabricación a medida. Gracias!`
  );
  
  if (!phone) {
    // If no phone number configured yet, trigger modal or alert helper
    return `javascript:alert("Número de WhatsApp corporativo pendiente de configuración. Puede contactarnos mediante el formulario o solicitar asistencia.");`;
  }
